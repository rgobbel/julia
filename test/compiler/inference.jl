...
Base.@noinfer function func_noinfer(@nospecialize a)
    c = isa(a, Function)
    inline_checker(c) # dynamic dispatch, preventing inlining
end

Base.@noinfer @inline function func_noinfer_inline(@nospecialize a)
    c = isa(a, Function)
    inline_checker(c) # dynamic dispatch, preventing inlining (but forced by the annotation)
end

Base.@noinfer Base.@constprop :aggressive function func_noinfer_constprop(c::Bool, @nospecialize a)
    if c
        return inline_checker(a) # dynamic dispatch, preventing inlining/constprop (but forced by the annotation)
    end
    return false
end
Base.@noinfer func_noinfer_constprop(@nospecialize a) = func_noinfer_constprop(false, a)
...
@testset "compilation annotations" begin
    ...
    @testset "@noinfer" begin
        # `@nospecialize` should suppress runtime dispatches of `nospecialize`
        @test call_func_itr(func_noinfer, itr_dispatchonly) == 2
        @test length(Base.specializations(only(methods((func_noinfer))))) == 1
        # `@noinfer` suppresses inference also
        @test call_func_itr(func_noinfer, itr_withinfernce) == 2
        @test length(Base.specializations(only(methods((func_noinfer))))) == 1
        @test !any(is_inline_checker, @get_code call_func_itr(func_noinfer, itr_dispatchonly))

        # `@noinfer` should allow inlinining
        @test call_func_itr(func_noinfer_inline, itr_dispatchonly) == 2
        @test length(Base.specializations(only(methods((func_noinfer_inline))))) == 1
        @test call_func_itr(func_noinfer_inline, itr_withinfernce) == 2
        @test length(Base.specializations(only(methods((func_noinfer_inline))))) == 1
        @test any(is_inline_checker, @get_code call_func_itr(func_noinfer_inline, itr_dispatchonly))

        # `@noinfer` should allow constprop
        @test Base.return_types((Any,)) do x
            Val(func_noinfer_constprop(x))
        end |> only == Val{false}
        @test call_func_itr(func_noinfer_constprop, itr_dispatchonly) == 0
        for m = methods(func_noinfer_constprop)
            @test length(Base.specializations(m)) == 1
        end
        @test call_func_itr(func_noinfer_constprop, itr_withinfernce) == 0
        for m = methods(func_noinfer_constprop)
            @test length(Base.specializations(m)) == 1
        end
    end
